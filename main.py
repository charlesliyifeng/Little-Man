...
def getAfternoon():
  ...
  while True:
    ...
    if t.hour<12:
      date[0]=t.hour+12
      date[3]=datetime.now().strftime("%A").startswith('S')
    else:
      date[0]=t.hour-12
      date[4]=datetime.now().strftime("%A").startswith('Sa')|datetime.now().strftime("%A").startswith('F')
    date[1]=t.minute
    date[2]=t.second
    time.sleep(1)
...
threading.Thread(target=getAfternoon).start()
